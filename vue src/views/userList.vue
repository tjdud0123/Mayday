  ...
  <div id="UserList">
    ...
    <div class="topBox">
        ...
        <div class="rightBox" v-on:click="orderAct">
            <div class="orderBy">{{this.$root.orderStd2}}</div>
    ...
    <div class="listContainer0">
        ...
        <div class="outContainer" v-for="(item, i) in ordered" v-bind:key="i" >
          <div class="list" v-if="$root.curCondition.includes(item.condition)" v-on:click="goToU(item.pk)">
              <div class="listImg2" v-bind:style='{ "background-image": "url(" + item.url + ")", "background-position": "center center", "background-size":"cover"}'></div>
              <div class="condition2">{{conText(item.condition)}}</div>
              <div class="Title">{{item.uTitle}}</div>
...
export default {
  ...
  data: function() {
     ...
     computed:{
        ordered : function(){
            return _.orderBy(this.$root.user_contents, this.$root.orderKey2, 'desc');
        }
    },
    ...
    methods:{
      ...
      getNum: function(n){
        ...
        goToU: function(i){
              this.$router.push({name: 'UserActDt', params: {userContentId : i, fw:3}})
          },
        ...
        orderClose: function(std){
            //order 순서바꾸기
            this.$root.orderStd2 = std
            let vm = this;

            if(std == '최신순'){
                vm.$root.orderKey2 = "date"
            }
            else{
                vm.$root.orderKey2 = "markNum"
            }
...
.outContainer:nth-child(even):first-child{
    /*margin-left: 3.46vw;*/
}
