<template>
  <div id="FromMe">
    ...
    <div class="topContainer center">
        ...
        <div class="title center">내가 쓴 답장</div>
        <img class='hamburger topRight' v-on:click="hamAct" src="css/hamburger_blk.png">
    </div>

    <!-- 리스트 -->

    <div class="listContainer">
        <div class="outContainer" v-for="(item, i) in $root.Replys" v-bind:key="i">
            <div class="list" v-if="(item.id == $root.currentInfo.id)" v-on:click="goTo(item.lpk, item.rpk)">
                <div class="listTitle2">{{item.date}}</div>
                <div class="listContent1">{{item.replyText.substring(0,25)}}...</div>
                <div class="listContent2">{{$root.Letters[item.lpk].writeText.substring(0,29)}}...</div>
            </div>
        </div>
    </div>
      ...
      <div class="popContainer pophamber" v-bind:style="{display:hamOn}">
        ...
        <div class="hamInfo">
            ...
            <div class="hamInfoBox">
                ...
                <div class="hamBox hamBox2" v-on:click="hamClose">
...
export default {
  ...
  data: function() {
    ...
    methods:{
        ...
        goTo:function(lpk, rpk){
          this.$router.push({name: 'FromMeDetail', params: {LetterId : lpk, ReplyId: rpk}})
        }
